...
const atob = require('atob'); // Untuk melakukan decoding base64
...
router.post('/', async (req, res) => {
    ...
    try {
        ...
        for (let signature of backdoorSignatures) {
            // Cek dalam konten asli
            if (responseData.includes(signature.pattern)) {
                backdoorDetails.push({
                    description: signature.description,
                    fileOrDir: signature.pattern
                });
            }

            // Cek dalam konten yang di-decode dari base64
            try {
                const decodedData = atob(responseData);
                if (decodedData.includes(signature.pattern)) {
                    backdoorDetails.push({
                        description: `${signature.description} (Base64 decoded)`,
                        fileOrDir: signature.pattern
                    });
                }
            } catch (error) {
                // Jika terjadi kesalahan pada decoding, abaikan saja
